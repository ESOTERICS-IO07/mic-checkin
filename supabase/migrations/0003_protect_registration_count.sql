-- Phase B1 Hardening: Protect events.registration_count using PostgreSQL column-level privileges.
-- Drop the trigger-based protection from the previous step.
DROP TRIGGER IF EXISTS events_protect_registration_count ON public.events;
DROP FUNCTION IF EXISTS public.protect_event_registration_count();

-- 1. Revoke table-level INSERT and UPDATE privileges on events from authenticated role
REVOKE INSERT, UPDATE ON public.events FROM authenticated;

-- Set default values so authenticated role doesn't need write permissions to populate them
...
BEGIN
  ...
  -- 7. Increment registration count (Executes as SECURITY DEFINER owner, bypassing column privileges)
